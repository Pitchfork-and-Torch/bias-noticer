...
import { NeutralitySparkline } from "../../components/NeutralitySparkline";
...
export function SidePanelApp() {
  ...
  async function feedback(
    ...
    const resolved: FeedbackKind =
      kind || (helpful ? "helpful" : "wrong");
    await sendToBackground({
      ...
      entry: {
        ...
        helpful: resolved === "helpful" || resolved === "too_weak",
        kind: resolved,
        severity: inst.severity,
        confidence: inst.confidence,
      },
    });
    const notes: Record<FeedbackKind, string> = {
      helpful: "Thanks — marked helpful. Local calibration noted.",
      wrong: "Thanks — marked wrong. Local thresholds will raise the bar.",
      too_strong: "Thanks — marked too strong.",
      too_weak: "Thanks — marked too weak.",
      missed: "Thanks — miss recorded for local calibration.",
    };
    setFeedbackNote(notes[resolved]);
    window.setTimeout(() => setFeedbackNote(null), 2500);
  }
  ...
  /** Tabs that work without an active scan */
  const alwaysOnTabs = new Set<PanelTab>([
    "outlets",
    "research",
    "academy",
    "feedback",
  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <main className="flex-1 space-y-4 p-4">
        ...
        {!analysis && !isScanning && !alwaysOnTabs.has(tab) && (
          <div className="bn-card p-6 text-center animate-fade-in">
            ...
            <p className="mt-2 text-[11px] text-slate-400">
              Tip: train in <strong>Academy</strong>, track outlets, or paste
              under Research — no scan required.
            </p>
        ...
        {analysis &&
          ...
          !isScanning &&
          (tab === "detected" || tab === "summary" || tab === "evidence") && (
          <div className="bn-card p-6 text-center animate-fade-in">
            <ScoreGauge
              score={displayNeutrality(analysis.summary)}
              size={88}
              className="mx-auto"
            />
        ...
        {/* Always-on surfaces: Academy (no scan required) */}
        {tab === "academy" && (
          <TechniqueAcademy
            instances={analysis?.instances ?? []}
            onJumpToSignal={(id) => {
              if (analysis) selectSignal(id);
            }}
          />
        )}

        <>
            {analysis && tab === "detected" && (
              <section className="space-y-3" role="tabpanel">
                {analysis.instances.length > 0 && (
                  <SignalRadar
                    instances={filtered.length ? filtered : analysis.instances}
                ...
                {selected && (
                  <DetailCard
                    ...
                    onFeedback={(h, k) => void feedback(selected, h, k)}
                  />
                )}
              </section>
            )}
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 text-[11px] text-slate-500">
                      ...
                      {analysis.pipeline
                        ? ` · ${analysis.pipeline.depth} (${analysis.pipeline.passes_run.join("→")})`
                        : ""}
                      {siteAvg != null
                    ...
                    <p className="mt-1 text-[10px] text-slate-400">
                      Grade uses calibrated neutrality when available — not a
                      political score.
                    </p>
                    ...
                    {analysis.summary.headline_body_note && (
                      <p className="mt-2 rounded-lg bg-amber-50 px-2 py-1.5 text-[11px] text-amber-900 dark:bg-amber-950/40 dark:text-amber-100">
                        Headline/body: {analysis.summary.headline_body_note}
                      </p>
                    )}
                  </div>
                </div>
                {analysis.missing_context &&
                  analysis.missing_context.length > 0 && (
                    <div className="rounded-xl border border-slate-200 p-3 dark:border-slate-700">
                      ...
                      <ul className="mt-2 space-y-1.5 text-[11px] leading-relaxed text-slate-600 dark:text-slate-300">
                    ...
                    <div className="bn-card space-y-3 p-4">
                      ...
                      {selectedJournalist.history.length > 0 && (
                        <div>
                          ...
                          <NeutralitySparkline
                            points={selectedJournalist.history.map((h) => ({
                              at: h.scannedAt,
                              neutrality: h.neutrality,
                              label: h.title,
                            }))}
                            width={220}
                            height={48}
                            title={`Neutrality trend for ${selectedJournalist.journalist.name}`}
                          />
                        </div>
                      )}
                    </div>
                    ...
                    <div className="bn-card space-y-3 p-4">
                      ...
                      {selectedOutlet.history.length > 0 && (
                        <div>
                          <div className="bn-label">Your scan timeline</div>
                          <NeutralitySparkline
                            points={selectedOutlet.history.map((h) => ({
                              at: h.scannedAt,
                              neutrality: h.neutrality,
                              label: h.title,
                            }))}
                            width={220}
                            height={48}
                            title={`Neutrality trend for ${selectedOutlet.outlet.host}`}
                          />
                        </div>
                      )}
                    </div>
              ...
              <section className="bn-card space-y-3 p-4 animate-fade-in" role="tabpanel">
                ...
                {selected ? (
                  ...
                  <p className="text-xs text-slate-500">
                    ...
                    it. Or open Technique Academy to practice spotting moves.
                  </p>
                )}
                <div className="flex flex-wrap gap-2">
                  <button
...
function DetailCard({
  ...
  onScroll: () => void;
  onRewrite: () => void;
  onFeedback: (helpful: boolean, kind?: FeedbackKind) => void;
}) {
  return (
    <section className="bn-card animate-slide-up space-y-3 p-4">
      ...
      {(selected.voice || selected.verification || selected.why_flagged) && (
        <div className="flex flex-wrap gap-2 text-[10px] text-slate-500">
          {selected.voice && selected.voice !== "unknown" && (
          ...
          {selected.origin_pass && (
            <span className="rounded-full bg-slate-100 px-2 py-0.5 dark:bg-slate-800">
              pass: {selected.origin_pass}
            </span>
          )}
        </div>
      )}
      <div>
        ...
        <p className="text-sm leading-relaxed">
          {selected.why_flagged ||
            selected.detailed_explanation ||
            selected.concise_explanation}
        </p>
        <p className="mt-1 text-xs text-slate-500">
          {getCategoryMeta(selected.bias_type).shortDefinition}
        </p>
        {selected.detailed_explanation && selected.why_flagged && (
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <button
          className="bn-btn-ghost text-xs"
          onClick={() => onFeedback(true, "helpful")}
        >
          Helpful
        </button>
        <button
          className="bn-btn-ghost text-xs"
          onClick={() => onFeedback(false, "wrong")}
        >
          Wrong
        </button>
        <button
          className="bn-btn-ghost text-xs"
          onClick={() => onFeedback(false, "too_strong")}
        >
          Too strong
        </button>
        <button
          className="bn-btn-ghost text-xs"
          onClick={() => onFeedback(true, "too_weak")}
        >
          Too weak
        </button>
      </div>
      <p className="text-[10px] text-slate-400">
        Feedback stays on this device and gently tunes local calibration — never
        a political score.
      </p>
    </section>
  );
