...
async function handleMessage(
  ...
  switch (message.type) {
    ...
    case "TEST_API_KEY": {
      const settings = await getSettings();
      const key = (message.apiKey ?? settings.apiKey ?? "").trim();
      // Persist a successful paste path even if Settings blur missed it
    ...
    case "SUBMIT_FEEDBACK": {
      const entry = await addFeedback(message.entry);
      try {
        await applyFeedbackToCalibration(message.entry);
      } catch {
        /* non-fatal */
      }
    ...
    case "GET_CALIBRATION": {
      ...
      return {
        ok: true,
        data: { store, summary: calibrationSummary(store) },
      };
    }
    case "GET_MEDIA_DIET": {
      const diet = await buildMediaDietSummary();
      return { ok: true, data: diet };
    }
    case "COMPARE_ANALYSES": {
      return { ok: true, data: compareAnalyses(message.a, message.b) };
    }
...
/** Attach structure + local context + calibrated neutrality to heuristic runs */
async function enrichHeuristic(
  analysis: BiasAnalysis,
  extract: PageExtract,
  settings?: { useLocalCalibration?: boolean }
): Promise<BiasAnalysis> {
  try {
    const structure = runStructurePass(extract);
    let next = attachLocalContext(analysis, extract, structure);
    if (settings?.useLocalCalibration !== false) {
      try {
        const cal = await loadCalibration();
        next = {
          ...next,
          instances: applyCalibrationToInstances(next.instances, cal, true),
        };
      } catch {
        /* keep uncalibrated instances */
      }
    }
    next = finalizeAnalysis(next, {
      structure,
      depth: "quick",
      passes: ["structure", "heuristic"],
      passNotes: ["Heuristic-only path (no API key or Grok unavailable)."],
    });
    return next;
  } catch {
    return analysis;
...
function attachLocalContext(
  analysis: BiasAnalysis,
  extract: PageExtract,
  structure: ReturnType<typeof runStructurePass>
): BiasAnalysis {
  const ctx = localContextHints(extract, structure, analysis.instances);
  return {
    ...analysis,
    ...
    summary: {
      ...analysis.summary,
      headline_body_note:
        ctx.headline_body_note || analysis.summary.headline_body_note,
    },
  };
}
...
async function runAnalysisForTab(
  ...
  if (analyzing.has(tabId)) {
    if (!force) {
      ...
      throw new Error(
        "A scan is already in progress for this tab. Wait a moment, or use Force re-scan."
      );
    }
    // Force re-scan: drop the prior lock (may orphan a hung fetch - preferred over stuck UI)
    analyzing.delete(tabId);
  }
  analyzing.add(tabId);

  // Hard safety: if the SW is killed mid-fetch, clear the lock so the next scan works
  const lockWatchdog = setTimeout(() => {
    if (analyzing.has(tabId)) {
      analyzing.delete(tabId);
      broadcastProgress(tabId, {
        stage: "error",
        message: "Scan timed out. Check API key / credits, then Force re-scan.",
        percent: 0,
      });
    }
  }, 150_000);
  ...
  try {
    ...
    broadcastProgress(tabId, {
      stage: "structure",
      message: "Pass 0: local structure...",
      percent: 30,
    });
    ...
    if (hasKey && settings.hybridQuickScan && !settings.neverSendFullText) {
      const provisional = await enrichHeuristic(
        runHeuristicAnalysis(extract),
        extract,
        settings
      );
      provisional.notes = [
        ...(provisional.notes || []),
        "Provisional local signals - multi-pass Grok analysis loading...",
      ];
      ...
      broadcastProgress(tabId, {
        ...
        message:
          settings.multiPass !== false
            ? "Multi-pass analysis with Grok..."
            : "Deep analysis with Grok...",
        percent: 55,
      });
    } else {
      broadcastProgress(tabId, {
    ...
    const progressCb = (p: {
      ...
      broadcastProgress(tabId, {
        stage: (p.stage as AnalysisProgress["stage"]) || "analyzing",
        message: p.message,
        percent: p.percent,
        pass: p.pass,
      });
    ...
    if (!hasKey || settings.neverSendFullText) {
        ...
        try {
          analysis = await analyzeWithGrok(
            limited,
            {
              ...
              // Limited mode: skip expensive multi-pass extras
              multiPass: false,
              analysisDepth: "quick",
            },
            progressCb
          ...
          analysis = await enrichHeuristic(
            runHeuristicAnalysis(extract),
            extract,
            settings
          );
      ...
      try {
        analysis = await analyzeWithGrok(extract, settings, progressCb);
      } catch (e) {
        analysis = await enrichHeuristic(
          runHeuristicAnalysis(extract),
          extract,
