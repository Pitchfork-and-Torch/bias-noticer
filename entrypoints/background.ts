...
async function runAnalysisForTab(
  ...
  try {
    ...
    if (hasKey && settings.hybridQuickScan && !settings.neverSendFullText) {
      const provisional = enrichHeuristic(runHeuristicAnalysis(extract), extract);
      provisional.notes = [
        ...(provisional.notes || []),
    ...
    if (!hasKey || settings.neverSendFullText) {
      if (settings.neverSendFullText && hasKey) {
        ...
        try {
          ...
          analysis = enrichHeuristic(runHeuristicAnalysis(extract), extract);
        }
      } else {
        analysis = enrichHeuristic(runHeuristicAnalysis(extract), extract);
      }
