...
export function OptionsApp() {
  ...
  async function save(partial: Partial<ExtensionSettings>) {
    // Optimistically merge for UI; server/storage is source of truth on success
  ...
  async function saveApiKey(raw?: string) {
    const key = (raw ?? settings.apiKey).trim();
    setSettings((prev) => ({ ...prev, apiKey: key }));
    setStatus(key ? "Saving key…" : "Clearing key…");
    const ok = await save({ apiKey: key });
    if (ok) setStatus(key ? "API key saved locally" : "API key cleared");
    return ok;
  }
  ...
  async function testKey() {
    ...
    if (!key) {
      setStatus("Enter an xAI API key first");
      return;
    }
    setTesting(true);
    setStatus("Saving & testing…");
    try {
      // Always persist the current field value before testing (fixes paste+blur race)
      const saved = await sendToBackground<ExtensionSettings>(
        { type: "SAVE_SETTINGS", settings: { apiKey: key } },
        { timeoutMs: 15_000 }
      );
      if (!saved.ok) {
        setStatus(saved.error || "Could not save key");
        return;
      }
          ...
          <section className="bn-card space-y-4 p-5">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                <input
                  id="apiKey"
                  className="bn-input min-w-[12rem] flex-1 font-mono"
                  ...
                  onBlur={(e) => {
                    // Use the input's live value — not React state — so paste+blur
                    // cannot overwrite storage with a stale empty key.
                    void saveApiKey(e.currentTarget.value);
                  }}
                  onKeyDown={(e) => {
                    if (e.key === "Enter") {
                      e.preventDefault();
              ...
              <p className="mt-1.5 text-[11px] text-slate-500">
                Paste your key, click <strong>Save key</strong>, then{" "}
                <strong>Test</strong>. Stored only in this browser (
                <code className="rounded bg-slate-100 px-1 dark:bg-slate-800">
          ...
          <section className="bn-card space-y-4 p-5">
            ...
            <div>
              ...
              <p className="mt-2 text-xs text-slate-500">
                ...
                Thorough sensitivity also prefers multi-pass depth when enabled.
              </p>
            </div>
            <div>
              <label className="flex cursor-pointer items-start gap-3">
                <input
                  type="checkbox"
                  className="mt-1"
                  checked={settings.multiPass !== false}
                  onChange={(e) => void save({ multiPass: e.target.checked })}
                />
                <span>
                  <span className="font-medium">Multi-pass analysis</span>
                  <span className="mt-0.5 block text-xs text-slate-500">
                    Pass 1 detect → Pass 2 verify spans &amp; voice. Improves
                    precision; uses extra API tokens when a key is set.
                  </span>
                </span>
              </label>
            </div>
            {settings.multiPass !== false && (
              <div>
                <label className="bn-label">Analysis depth</label>
                <div className="flex flex-wrap gap-2">
                  {(
                    [
                      ["quick", "Quick"],
                      ["standard", "Standard"],
                      ["thorough", "Thorough + context"],
                    ] as const
                  ).map(([id, label]) => (
                    <button
                      key={id}
                      className={
                        (settings.analysisDepth || "standard") === id
                          ? "bn-btn-primary"
                          : "bn-btn-secondary"
                      }
                      onClick={() => void save({ analysisDepth: id })}
                    >
                      {label}
                    </button>
                  ))}
                ...
                <p className="mt-2 text-xs text-slate-500">
                  Thorough runs Pass 3 (missing stakeholders / headline-body).
                  Standard uses free local structural hints for Pass 3.
                </p>
              </div>
            )}
            <div>
              <label className="flex cursor-pointer items-start gap-3">
                <input
                  type="checkbox"
                  className="mt-1"
                  checked={settings.useLocalCalibration !== false}
                  onChange={(e) =>
                    void save({ useLocalCalibration: e.target.checked })
                  }
                />
                <span>
                  <span className="font-medium">Local calibration</span>
                  <span className="mt-0.5 block text-xs text-slate-500">
                    Use your Wrong / Too strong feedback to gently raise or lower
                    thresholds per technique. Never leaves this device.
                  </span>
                </span>
              </label>
            </div>
...
function Onboarding({ onDone }: { onDone: () => void }) {
  ...
  const steps = [
    ...
    },
    {
      ...
      body: "Highlights mark rhetorical techniques, not “wrongthink.” Hover for a quick explanation; open the side panel for Summary, Evidence, Signal Radar, Technique Academy, and Feedback.",
    },
    {
