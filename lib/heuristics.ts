...
const RULES: Rule[] = [
  ...
  },
  {
    ...
    pattern:
      /\b(sources say|it is widely known|everyone knows|clearly|undeniabl[ye]|it is undeniable|without question|proves that)\b/gi,
    severity: 2,
    confidence: 0.5,
  ...
  },
  {
    type: "straw_man",
    pattern:
      /\b(they want (chaos|open borders|to destroy)|critics want zero|their entire (platform|agenda) is)\b/gi,
    severity: 3,
    confidence: 0.5,
    explanation: "Possible misrepresentation of an opposing position.",
    detailed:
      "Collapsing a complex view into an extreme caricature makes rebuttal easy and understanding hard.",
    counter: "Find the strongest formulation of the opposing argument (steelman).",
    alt: "Quote the opponent's actual claim before critiquing it.",
  },
  {
    type: "ad_hominem",
    pattern:
      /\b(only a (fool|idiot|moron)|paid shill|so-called expert|how dare (he|she|they))\b/gi,
    severity: 3,
    confidence: 0.55,
    explanation: "Attack on the person may replace engagement with the claim.",
    detailed:
      "Insults and motive smears can short-circuit evaluation of evidence.",
    counter: "Strip the insult and test the underlying argument's evidence chain.",
    alt: "Address the claim's evidence without attacking identity or character.",
  },
  {
    type: "slippery_slope",
    pattern:
      /\b(inevitably lead|slippery slope|total tyranny is inevitable|opens the floodgates to)\b/gi,
    severity: 2,
    confidence: 0.5,
    explanation: "Cascade claim may skip intermediate causal steps.",
    detailed:
      "Inevitability language needs a evidenced chain from step A to extreme Z.",
    counter: "List each required step and demand evidence for the chain.",
    alt: "Argue each causal step or drop inevitability language.",
  },
  {
    type: "bandwagon",
    pattern:
      /\b(everyone knows|growing consensus that|most people agree|the only sensible)\b/gi,
    severity: 2,
    confidence: 0.45,
    explanation: "Popularity may be used as a substitute for evidence.",
    detailed:
      "Social consensus is information, not proof. Ask what independent evidence shows.",
    counter: "Replace popularity claims with primary data or methods.",
    alt: "State the evidence; treat popularity as secondary context only.",
  },
  {
    type: "euphemism_dysphemism",
    pattern:
      /\b(rightsizing|enhanced interrogation|collateral damage|regime change|thugs|illegals)\b/gi,
    severity: 2,
    confidence: 0.5,
    explanation: "Soft or harsh labels may reframe the same underlying event.",
    detailed:
      "Euphemism softens; dysphemism harshens. Translate both into the same plain description.",
    counter: "Use consistent plain language; check legal or technical definitions.",
    alt: "Name the concrete action without moralized shorthand.",
  },
  {
    type: "poisoning_the_well",
    pattern:
      /\b(before you (listen|believe)|known liar|in the pocket of|don't trust anything they)\b/gi,
    severity: 3,
    confidence: 0.5,
    explanation: "Pre-emptive discrediting may invite discounting later evidence.",
    detailed:
      "Interests can be disclosed; pre-dismissal of all evidence from a source is well-poisoning.",
    counter: "Disclose interests, then still test methods and data on the merits.",
    alt: "Note conflicts of interest without voiding all subsequent evidence.",
  },
  {
    type: "appeal_to_authority",
    pattern:
      /\b(because (the )?[a-z]{2,20} said so|as an? (expert|scientist|doctor|ceo)\b|trust me,? I('m| am) an expert)\b/gi,
    severity: 2,
    confidence: 0.5,
    explanation: "A title or named authority may be substituting for evidence.",
    detailed:
      "Expertise can be relevant. It becomes a technique when 'they said so' replaces methods, data, or a checkable claim.",
    counter: "Ask what measurement, document, or method the authority is relying on.",
    alt: "Cite the underlying evidence; treat the title as context, not proof.",
  },
  {
    type: "hasty_generalization",
    pattern:
      /\b(this one .{0,48}proves the entire|one (viral )?(clip|example|anecdote|case) proves|proves the entire (profession|generation|country|party|industry))\b/gi,
    severity: 2,
    confidence: 0.5,
    explanation: "A single example may be stretched into a sweeping claim.",
    detailed:
      "Anecdotes can illustrate. They do not by themselves prove a rule about a whole group.",
    counter: "Ask for base rates, sample size, and counter-examples before the generalization.",
    alt: "Describe the single case; separate it from any claim about the whole class.",
  },
];
