...
/**
 * Clamp a neutrality-like value to 0 - 100.
...
export function neutralityToGrade(score: number): GradeInfo {
  const s = finiteNeutrality(score, 0);
...
export function displayNeutrality(summary: {
  ...
  if (
    ...
    Number.isFinite(summary.calibrated_neutrality)
  ) {
    return finiteNeutrality(summary.calibrated_neutrality, 0);
  }
  return finiteNeutrality(summary.neutrality_score, 0);
}
...
export function computeCalibratedNeutrality(
  ...
  const raw = finiteNeutrality(modelScore, 50);
  if (!instances.length) {
