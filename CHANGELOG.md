...
## 2.1.1 - Honest zero, production gold

Patch on 2.1. Scoring honesty and local heuristics. Host permissions unchanged.

### Scoring
- Model neutrality **0 stays 0**. Grok JSON parse no longer treats 0 as missing (`|| 50`)
- `finiteNeutrality` is the single 0-100 clamp; missing or NaN still fall back to 50
- Toolbar badge, outlet/journalist scoreboards, and the popup gauge use `displayNeutrality` (calibrated when present)

### Local calibration
- Heuristic path loads on-device calibration, then finalize (same as the Grok path)
- Two "wrong" marks on a type can drop a typical 0.50 heuristic hit (the old 0.35 floor never fired)

### Gold / heuristics
- Production gold suite **20/20** against `lib/heuristics.ts` (`npm run validate:score`)
- Offline rules for appeal-to-authority and hasty generalization; "it is undeniable" hits unsubstantiated-claim
- Media-diet recurring technique counts no longer double-count the same scan (`signals` preferred over `topTypes`)

### Honesty
- Chrome Web Store remains a **human zip upload** (`docs/store/CWS-DROP.md`). This GitHub cut is `v2.1.1`.
