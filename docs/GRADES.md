...
Raw model `neutrality_score` is retained for transparency; `calibrated_neutrality` is what letter badges, the toolbar badge color, outlet/journalist scoreboards, and media-diet averages use via `displayNeutrality()`. The same calibrated value is what gets rolled into outlet and journalist averages, scan history, and the toolbar badge, so one article gets one grade everywhere.

`0` is a valid neutrality (extreme framing load). Scoring never treats `0` as "missing" (`|| 50` is a bug).
